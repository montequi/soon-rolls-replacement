...
class millingmachine:
    def __init__(self,cod_maquina,tmill,tmax):
...
class grindingmachine:
    def __init__(self,cod_maquina,tgrind,tmax):
        self.cod_maquina = cod_maquina
        self.tgrind = tgrind
        self.tmax = tmax

#jobs of a milling machine
class millingmachinework:
    def __init__(self,roll,positions,tinicio,tfin,cod_maquina):
        self.roll=roll
...
#jobs of a grinding machine
class grindingmachinework:
    def __init__(self,roll,positions,tinicio,tfin,cod_maquina):
        self.roll=roll
        self.positions=positions
        self.tinicio=tinicio
...
class Trabajos:
    def __init__(self, toneladaslaminadas, calidadmaterial, cajas, duracion):
...
def anadirelementoarchivo(rodillo, posicionescil, posicionusada, toneladas, geometrias, calidades, model, dureza):
    ...
    dframe = pd.DataFrame(frame)
    #with the dataframe it is calculated the reduction
    predictions = model.predict(dframe)
...
if __name__ == '__main__':
    ...
    numeromillingmachine = 2
    #number of grinding machines
    numerogrindingmachine = 3
    #grinding machines
    grindingmachines = []
    #milling machines
    millingmachines = []
    #jobs for the grinding machines
    grindingmachineworks = [[] for a in range(numerogrindingmachine)]
    #jobs for the milling machines
    millingmachineworks = [[] for a in range(numeromillingmachine)]
    #origin of times
    origentiemposgrinmill = 0
    #time it lasts to grind
    tgrind = 200
    #time it lasts to mill
    tmill = 200
    #the milling machines are created
    for element in range(numeromillingmachine):
        millingmachines.append(millingmachine(element,tmill,0))
    #the grinding machines are created
    for element in range(numerogrindingmachine):
        grindingmachines.append(grindingmachine(element,tgrind,0))
    ...
    for histoelement in range(0, 1):
        ...
        datoscostes = []
        #jobs are repeated in a loop in this case 100 times
        repeticionestrabajos = 100
        #index of the job to be performed
        indicetrabajo = 0
        # data from the jobs, geometries, qualities, shapes, measures and costs is loaded from the configuration file
        with open('trabajos.json') as json_file:
            data = json.load(json_file)
            for p in data['TrabajosLaminacionDuracion']:
                cajas = []
                for p2 in p['cajas']:
                    cajas.append(CajasTrabajos(p2['caja'], p2['geometria'], p2['tallado']))
                trabajos.append(Trabajos(p['toneladaslaminadas'], p['calidadmaterial'], cajas,p['duracion']))
        ...
        #the repeated jobs are put together in an array to perform them
        trabajos2 = []
        for i in range(repeticionestrabajos):
            trabajos2 = trabajos2 + trabajos
        trabajos = trabajos2
        #damaged rolls
        rotosac = []
        rotostc = []
        ...
        for trabajo in trabajos:
            print("cuantos tengo")
            print(len(cilindrosgenerados))
            print(len(posicionesgeneradas))
            ...
            with open('trabajos.json') as json_file:
                ...
                for p in data['restricciones'][indicetrabajo % int(len(trabajos) / repeticionestrabajos)]:
            ...
            diametroagentes = [[] for a in range(numeroagentes)]
            #diameter reductions of the agents
            reduccionesagentes = [[] for a in range(numeroagentes)]
            #volume reductions of the agents
            reduccionesvolumenagentes = [[] for a in range(numeroagentes)]
            #sum of the volume reductions of the agents
            reduccionestotalesvolumenagentes = [[] for a in range(numeroagentes)]
            # monetary cost of the agents
            costesagentes = [[] for a in range(numeroagentes)]
            #sum of the monetary cost of the agents
            costestotalesagentes = [[] for a in range(numeroagentes)]
            #average diameter of the agents
            mediadiametros = [[] for a in range(numeroagentes)]
            #average reductions of the agents
            mediareducciones = [[] for a in range(numeroagentes)]
            #average volume reductions of the agents
            mediareduccionvolumen = [[] for a in range(numeroagentes)]
            #average costs of the agents
            mediacostes = [[] for a in range(numeroagentes)]
            #standard deviation of the diameter of the agents
            desviaciontipicadiametros = [[] for a in range(numeroagentes)]
            #standard deviation of the diameter reductions of the agents
            desviaciontipicareducciones = [[] for a in range(numeroagentes)]
            #standard deviation of the volume reductions of the agents
            desviaciontipicareduccionesvolumen = [[] for a in range(numeroagentes)]
            # standard deviation of the costs of the agents
            desviacioncostes = [[] for a in range(numeroagentes)]
            # data from the rolls assembled on each of the agents
            datoscilindroagentes = [[] for a in range(numeroagentes)]
            ...
            while contadorsoluciones < numeroagentes and numeroejecuciones < 100:
                ...
                if len(poblacioninicial) == len(cajas):
                    ...
                    #store of the rolls used in this solution to later check that they are not repeated among solutions
            ...
            if contadorsoluciones == numeroagentes:
                ...
                while reduccionprevia > reduccionnueva and (reduccionprevia - reduccionnueva) > 0.01 and not terminar:
                    ...
                    #the compatible rolls with each stand are auctioned
                    for cajita in miscompatibles:
                        ...
                        for rodillo in cajita.compatibles:
                            ...
                            # datos que al final se guardan del rodillo para el agente y caja ganadores
                            ...
                            if len(rodilloscoincidentes) == 0 and len(posicionescil) > 0:
                                ...
                                contador = 0
                                #loop for the different solutions
                                ...
                                if poscajaarray == 0 and rodillo in miscompatibles[poscajaarray + 1].compatibles:
                                    ...
                                    contador = 0
                                    #loop for each of the solutions
                                    for solucion in soluciones:
                                        posicionescil2 = list(
                                ...
                                if mejoracostes[posicionmaximo] > 0:
                                    ...
                                    for rodillito in soluciones[posicionmaximo]:
                                        ...
                                        if cuentacajas == poscajaarray + cajaquemejora:
                                            ...
                                            # acumuladocostes = acumuladocostes + redss
                    ...
                    for element in range(numeroagentes):
            ...
            #the origin of times is increased with each job
            origentiemposgrinmill=origentiemposgrinmill + trabajo.duracion
            #check if tmax(where the next rectified will start) of each machine is less than the origin of times in that case it is replaced
            for elementito in range(numeromillingmachine):
                if millingmachines[elementito].tmax < origentiemposgrinmill:
                    millingmachines[elementito].tmax = origentiemposgrinmill
            #check if tmax(where the next rectified will start) of each machine is less than the origin of times in that case it is replaced
            for elementito in range(numerogrindingmachine):
                if grindingmachines[elementito].tmax < origentiemposgrinmill:
                    grindingmachines[elementito].tmax = origentiemposgrinmill
            if not terminar:
                tfinal = time.time()
                cadenita = cadenita[:len(cadenita) - 1]
                ...
                #when every position has been marked as used, they are added to the grinding and milling machines and once they
                #have passed both, they are marked as active, the tons put to 0 and the reconditioning diameter (the one it is indicated
                #for each position once it rolls) to the minimum of both
                dic = {}
                for element2 in range(len(solucionfinal)):
                    ...
                    if len(posicionescilindrin) == 0 and solucionfinal[element2].cod_estado == 1:
                        ...
                        #check the machine that first rest free to assign the roll to recondition to it
                        minimotmax = -1
                        posminimotmax = -1
                        #it is checked for grinding
                        for elementito in range(numeromillingmachine):
                            if millingmachines[elementito].tmax < minimotmax or minimotmax == -1:
                                minimotmax = millingmachines[elementito].tmax
                                posminimotmax = millingmachines[elementito].cod_maquina
                        minimotmax2 = -1
                        posminimotmax2 = -1
                        #it is checked for milling
                        for elementito in range(numerogrindingmachine):
                            if grindingmachines[elementito].tmax < minimotmax2 or minimotmax2 == -1:
                                minimotmax2 = grindingmachines[elementito].tmax
                                posminimotmax2 = grindingmachines[elementito].cod_maquina
                        #array of the positions to recondition
                        posicionesrectificar = []
                        for elementin in posicionescilindrin2:
                            ...
                            posicionesrectificar.append(elementin)
                        cilindrosgenerados = list(
                            filter(lambda x: x.cod_interno != solucionfinal[element2].cod_interno, cilindrosgenerados))
                        #the diameter of the roll is updated
                        solucionfinal[element2].diam_actual = minimo
                        valorg1=solucionfinal[element2]
                        valorg2=posicionesrectificar
                        valorg3=grindingmachines[posminimotmax2].tmax
                        valorg4=grindingmachines[posminimotmax2].tmax + grindingmachines[posminimotmax2].tgrind
                        valorg5=posminimotmax2
                        grindingmachineworks[posminimotmax2].append(grindingmachinework(valorg1,valorg2,valorg3,valorg4,valorg5))
                        grindingmachines[posminimotmax2].tmax = grindingmachines[posminimotmax2].tmax + grindingmachines[posminimotmax2].tgrind
                        if grindingmachines[posminimotmax2].tmax > millingmachines[posminimotmax].tmax:
                            millingmachines[posminimotmax].tmax = grindingmachines[posminimotmax2].tmax + millingmachines[posminimotmax].tmill
                        millingmachineworks[posminimotmax].append(millingmachinework(solucionfinal[element2],posicionesrectificar,millingmachines[posminimotmax].tmax - millingmachines[posminimotmax].tmill,millingmachines[posminimotmax].tmax,posminimotmax))
                poswork=0
                #the rolls that have been reconditioned are added to the pool of available rolls
                #the rolls that have ended its treatment in a machine are removed from the queue of that machine
                for elementorect in millingmachineworks:
                    for elementorect2 in elementorect:
                        if elementorect2.tfin<origentiemposgrinmill:
                            for posaanadir in elementorect2.positions:
                                posicionesgeneradas.append(posaanadir)
                            cilindrosgenerados.append(elementorect2.roll)
                            millingmachineworks[poswork].remove(elementorect2)
                    poswork=poswork+1
                poswork=0
                for elementorect in grindingmachineworks:
                    for elementorect2 in elementorect:
                        if elementorect2.tfin<origentiemposgrinmill:
                            grindingmachineworks[poswork].remove(elementorect2)
                    poswork=poswork+1
